...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    var newBoundary = new Boundaries
    newBoundary.minX = Math.min(a.minX, b.minX)
    newBoundary.maxX = Math.max(a.maxX, b.maxX)
    newBoundary.minY = Math.min(a.minY, b.minY)
    newBoundary.maxY = Math.max(a.maxY, b.maxY)
    newBoundary
  }
