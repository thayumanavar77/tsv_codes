...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    output(0) = 0
    var (maxSoFar,currentHeight) = (0.0f,0.0f)

    for (i <- 1 until input.length) {
      currentHeight = input(i) / i
      maxSoFar = max(maxSoFar, currentHeight)
      output(i) = maxSoFar
    }
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if ((end - from) <= threshold)
      ...
      var middle = from + (end - from)/2
      val (ls,rs) = parallel(upsweep(input, from, middle, threshold),
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = {
      tree match {
        ...
        case Node(ls, rs) =>
          parallel(downsweep(input, output, startingAngle, ls),
                   downsweep(input, output, ls.maxPrevious, rs))
      }
  }
