...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
           ...
           if (initial.isEmpty) Stream.Empty
            ...
            val newPaths = newNeighborsOnly(neighborsWithHistory(initial.head._1, initial.head._2), 
                                            explored + initial.head._1)          
            newPaths ++ from(initial.tail ++ newPaths, explored + initial.head._1)
           }
