...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    List() :: (for {
      ...
      c <- 1 to count
      rest <- combinations(occurrences.filter(w => w._1 > ch))
    } yield List((ch, c)) ++ rest)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (x.toMap foldLeft (Map[Char, Int]())) ((acc, xelem) =>
      acc.updated(xelem._1, xelem._2 - y.toMap.withDefaultValue(0)(xelem._1)))
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsAux(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {c <- combinations(occurrences)
             w <- dictionaryByOccurrences.getOrElse(c, Nil)
             s <- sentenceAnagramsAux(subtract(occurrences, wordOccurrences(w)))
             if !c.isEmpty
        }
          yield w :: s
      }
